...
```mermaid
flowchart TD
    data_gen["Data Generation (Synthetic Engagement CSV)"]
    ml_train["ML Training (Random Forest Model)"]
    saved_models["Saved Models (encoder & regressor joblib)"]
    notebook["Interactive Jupyter Notebook (Data Gen, ML, Viz)"]
    dashboard["Interactive Dashboard HTML (Plotly + ipywidgets)"]
